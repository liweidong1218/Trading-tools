...
class mrsPreprocess:
    def __init__(self, ticker_df):
        self.df = self.create_variable(ticker_df)
        self.train = None
        self.test = None

    #construct variables
    def create_variable(self, ticker_df):
        """
        ...
        """
        self.assert_columns(ticker_df)
        df = ticker_df.copy()
        #endogenous
        df['pricechange'] = df['adj close'].diff()
    ...
    def split_train_test(self, testNum = 80):
        self.train = self.df.iloc[:-testNum]
        self.test = self.df.iloc[-testNum:]
